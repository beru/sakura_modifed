...
void EditView::OnLBUTTONDOWN(WPARAM fwKeys, int _xPos , int _yPos)
{
	...
	if (tripleClickMode) {
		...
		nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::TripleClick].m_nFuncCodeArr[GetCtrlKeyState()];
		if (nFuncID == 0) {
	...
	if (!tripleClickMode && GetDllShareData().m_common.edit.m_bUseOLE_DragDrop) {
		if (GetDllShareData().m_common.edit.m_bUseOLE_DropSource) {		// OLEによるドラッグ元にするか
			// 行選択エリアをドラッグした
...
//@@@ 2002.01.08 YAZAKI フリーカーソルOFFで複数行選択し、行の後ろをクリックするとそこにキャレットが置かれてしまうバグ修正
				...
				// 選択範囲のデータを取得
				if (GetSelectedData(&memCurText, false, NULL, false, GetDllShareData().m_common.edit.m_bAddCRLFWhenCopy)) {
					DWORD dwEffects;
...
normal_action:;
	...
	if (GetKeyState_Alt() && !tripleClickMode) {
		...
		if (ptMouse.x < GetTextArea().GetAreaLeft() && !GetKeyState_Shift()) {
			...
			// URLがクリックされたら選択するか
			if (GetDllShareData().m_common.edit.m_bSelectClickedURL) {
...
// novice 2004/10/10
	...
	nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::RightClick].m_nFuncCodeArr[nIdx];
	if (nFuncID != 0) {
		// コマンドコードによる処理振り分け
...
void EditView::OnMBUTTONDOWN(WPARAM fwKeys, int xPos , int yPos)
{
	...
	if (GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::CenterClick].m_nFuncCodeArr[nIdx] == F_AUTOSCROLL) {
		if (m_nAutoScrollMode) {
			AutoScrollExit();
			return;
		}else {
...
void EditView::OnMBUTTONUP(WPARAM fwKeys, int xPos , int yPos)
{
	...
	// ホイール操作によるページスクロールあり
	if (GetDllShareData().m_common.general.m_nPageScrollByWheel == (int)MouseFunctionType::CenterClick &&
	...
	// ホイール操作によるページスクロールあり
	if (GetDllShareData().m_common.general.m_nHorizontalScrollByWheel == (int)MouseFunctionType::CenterClick &&
	...
	nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::CenterClick].m_nFuncCodeArr[nIdx];
...
void EditView::OnXLBUTTONUP(WPARAM fwKeys, int xPos , int yPos)
{
	...
	// ホイール操作によるページスクロールあり
	if (GetDllShareData().m_common.general.m_nPageScrollByWheel == (int)MouseFunctionType::LeftSideClick &&
	    m_pEditWnd->IsPageScrollByWheel()
	) {
		m_pEditWnd->SetPageScrollByWheel(FALSE);
		return;
	...
	// ホイール操作によるページスクロールあり
	if (GetDllShareData().m_common.general.m_nHorizontalScrollByWheel == (int)MouseFunctionType::LeftSideClick &&
	    m_pEditWnd->IsHScrollByWheel()
	) {
		m_pEditWnd->SetHScrollByWheel(FALSE);
		return;
	...
	nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::LeftSideClick].m_nFuncCodeArr[nIdx];
	if (nFuncID != 0) {
		// コマンドコードによる処理振り分け
		//	May 19, 2006 genta マウスからのメッセージはCMD_FROM_MOUSEを上位ビットに入れて送る
		::PostMessage(::GetParent(m_hwndParent), WM_COMMAND, MAKELONG(nFuncID, CMD_FROM_MOUSE),  (LPARAM)NULL);
...
void EditView::OnXRBUTTONUP(WPARAM fwKeys, int xPos , int yPos)
{
	...
	// ホイール操作によるページスクロールあり
	if (GetDllShareData().m_common.general.m_nPageScrollByWheel == (int)MouseFunctionType::RightSideClick &&
	    m_pEditWnd->IsPageScrollByWheel()
	) {
	...
	// ホイール操作によるページスクロールあり
	if (GetDllShareData().m_common.general.m_nHorizontalScrollByWheel == (int)MouseFunctionType::RightSideClick &&
	    m_pEditWnd->IsHScrollByWheel()
	) {
	...
	nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::RightSideClick].m_nFuncCodeArr[nIdx];
	if (nFuncID != 0) {
		// コマンドコードによる処理振り分け
		//	May 19, 2006 genta マウスからのメッセージはCMD_FROM_MOUSEを上位ビットに入れて送る
		::PostMessage(::GetParent(m_hwndParent), WM_COMMAND, MAKELONG(nFuncID, CMD_FROM_MOUSE),  (LPARAM)NULL);
	...
	if (!GetSelectionInfo().IsMouseSelecting()) {
		...
		if (m_bDragMode) {
			...
			if (ptMouse.x < GetTextArea().GetAreaLeft() || ptMouse.y < GetTextArea().GetAreaTop()) {	//	2002/2/10 aroka
				...
				&& GetDllShareData().m_common.edit.m_bUseOLE_DragDrop	// OLEによるドラッグ & ドロップを使う
				&& GetDllShareData().m_common.edit.m_bUseOLE_DropSource	// OLEによるドラッグ元にするか
...
//	MYTRACE(_T("EditView::DispatchEvent() WM_MOUSEWHEEL fwKeys=%xh zDelta=%d xPos=%d yPos=%d \n"), fwKeys, zDelta, xPos, yPos);
...

	{
		...
		if (nCmdFuncID == F_0) {
			// 通常スクロールの時だけ適用
			bHorizontal = IsSpecialScrollMode(GetDllShareData().m_common.general.m_nHorizontalScrollByWheel);
			bKeyPageScroll = IsSpecialScrollMode(GetDllShareData().m_common.general.m_nPageScrollByWheel);
		}
		...
		if (nFuncID != F_0) {
			...
			if (nScrollCode == SB_LINEUP) {
				nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::WheelLeft].m_nFuncCodeArr[nIdx];
			}else {
				nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::WheelRight].m_nFuncCodeArr[nIdx];
			}
		}else {
			if (nScrollCode == SB_LINEUP) {
				nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::WheelUp].m_nFuncCodeArr[nIdx];
			}else {
				nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::WheelDown].m_nFuncCodeArr[nIdx];
			}
		}
		bool bExecCmd = false;
		{
		...
		const bool bSmooth = !! GetDllShareData().m_common.general.m_nRepeatedScroll_Smooth;
...
void EditView::OnLBUTTONDBLCLK(WPARAM fwKeys, int _xPos , int _yPos)
{
	...
	if (! m_dwTripleClickCheck) {
		...
		if (1
			...
			&& GetDllShareData().m_common.search.m_bGTJW_LDBLCLK
		) {
			// タグジャンプ機能
...
// novice 2004/10/10
	...
	EFunctionCode nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[
	...
	if (m_dwTripleClickCheck) {
		...
		if (!nFuncID) {
			m_dwTripleClickCheck = 0;	// トリプルクリックチェック OFF
			nFuncID = GetDllShareData().m_common.keyBind.m_pKeyNameArr[(int)MouseFunctionType::DoubleClick].m_nFuncCodeArr[nIdx];
...
STDMETHODIMP EditView::DragEnter(LPDATAOBJECT pDataObject, DWORD dwKeyState, POINTL pt, LPDWORD pdwEffect)
{
	...
	if (!GetDllShareData().m_common.edit.m_bUseOLE_DragDrop) return E_UNEXPECTED;
