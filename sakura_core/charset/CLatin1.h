...
public:
	// CodeBaseインターフェース
	EConvertResult CodeToUnicode(const Memory& cSrc, NativeW* pDst){ return Latin1ToUnicode(cSrc, pDst); }	//!< 特定コード → UNICODE    変換
	EConvertResult UnicodeToCode(const NativeW& cSrc, Memory* pDst){ return UnicodeToLatin1(cSrc, pDst); }	//!< UNICODE    → 特定コード 変換
...
public:
	// 実装
	static EConvertResult Latin1ToUnicode(const Memory& cSrc, NativeW* pDstMem);		// Latin1   → Unicodeコード変換
	static EConvertResult UnicodeToLatin1(const NativeW& cSrc, Memory* pDstMem);		// Unicode  → Latin1コード変換
