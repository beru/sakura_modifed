...
public:
	...
	static EConvertResult _UTF8ToUnicode( const Memory& cSrc, NativeW* pDstMem, bool bCESU8Mode );
	static EConvertResult _UnicodeToUTF8( const NativeW& cSrc, Memory* pDstMem, bool bCESU8Mode );
	inline static EConvertResult UTF8ToUnicode( const Memory& cSrc, NativeW* pDst ){ return _UTF8ToUnicode(cSrc, pDst, false); }	// UTF-8 -> Unicodeコード変換
	inline static EConvertResult CESU8ToUnicode( const Memory& cSrc, NativeW* pDst ){ return _UTF8ToUnicode(cSrc, pDst, true); }	// CESU-8 -> Unicodeコード変換
	inline static EConvertResult UnicodeToUTF8( const NativeW& cSrc, Memory* pDst ){ return  _UnicodeToUTF8(cSrc, pDst, false); }	// Unicode → UTF-8コード変換
	inline static EConvertResult UnicodeToCESU8( const NativeW& cSrc, Memory* pDst ){ return _UnicodeToUTF8(cSrc, pDst, true); }	// Unicode → CESU-8コード変換
