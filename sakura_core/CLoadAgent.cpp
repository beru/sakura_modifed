...
CallbackResultType LoadAgent::OnCheckLoad(LoadInfo* pLoadInfo)
{
	...
	if (IsDirectory(pLoadInfo->filePath)) {
		...
		bool bDlgResult = pcDoc->m_docFileOperation.OpenFileDialog(
...
next:
	...
	do {
		...
		bool bLock = (pLoadInfo->IsSamePath(pcDoc->m_docFile.GetFilePath()) && pcDoc->m_docFile.IsFileLocking());
		if (bLock) {
...
LoadResultType LoadAgent::OnLoad(const LoadInfo& sLoadInfo)
{
	...
	// パスを確定
	pcDoc->SetFilePathAndIcon( sLoadInfo.filePath );

	// 文書種別確定
	pcDoc->m_docType.SetDocumentType( sLoadInfo.nType, true );
	pcDoc->m_pEditWnd->m_pViewFontMiniMap->UpdateFont(&pcDoc->m_pEditWnd->GetLogfont());
	InitCharWidthCache( pcDoc->m_pEditWnd->m_pViewFontMiniMap->GetLogfont(), CharWidthFontMode::MiniMap );
	...
	pcDoc->m_pEditWnd->m_pViewFont->UpdateFont(&pcDoc->m_pEditWnd->GetLogfont());

	// 起動と同時に読む場合は予めアウトライン解析画面を配置しておく
	// （ファイル読み込み開始とともにビューが表示されるので、あとで配置すると画面のちらつきが大きいの）
	if (!pcDoc->m_pEditWnd->m_dlgFuncList.m_bEditWndReady) {
		pcDoc->m_pEditWnd->m_dlgFuncList.Refresh();
		HWND hEditWnd = pcDoc->m_pEditWnd->GetHwnd();
		if (!::IsIconic( hEditWnd ) && pcDoc->m_pEditWnd->m_dlgFuncList.GetHwnd()) {
			RECT rc;
	...
	if (fexist(sLoadInfo.filePath)) {
		...
		const TypeConfig& types = pcDoc->m_docType.GetDocumentAttribute();
		pcDoc->m_docFile.SetCodeSet( sLoadInfo.eCharCode, 
	...
	const TypeConfig& ref = pcDoc->m_docType.GetDocumentAttribute();
	...
	ProgressSubject* pOld = EditApp::getInstance()->m_pVisualProgress->ProgressListener::Listen(&pcDoc->m_layoutMgr);
	pcDoc->m_layoutMgr.SetLayoutInfo( true, ref, ref.m_nTabSpace, nMaxLineKetas );
...
void LoadAgent::OnAfterLoad(const LoadInfo& sLoadInfo)
{
	...
	// テキストの折り返し方法を初期化
	pcDoc->m_nTextWrapMethodCur = pcDoc->m_docType.GetDocumentAttribute().m_nTextWrapMethod;	// 折り返し方法
	...
	if (pcDoc->m_nTextWrapMethodCur == (int)TextWrappingMethod::NoWrapping) {
		pcDoc->m_layoutMgr.CalculateTextWidth();		// テキスト最大幅を算出する
	}else {
		pcDoc->m_layoutMgr.ClearLayoutLineWidth();		// 各行のレイアウト行長の記憶をクリアする
	}
}
