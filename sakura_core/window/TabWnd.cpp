...
{
	if (bForeground) {
		// 対象ウィンドウのプロセスが既にフォアグラウンドなら切替え済みなので何もしないでおく
		DWORD dwPid1, dwPid2;
		...
		if (::IsIconic(hwnd)) {
			::ShowWindow(hwnd, SW_RESTORE);	// アイコン時は元のサイズに戻す
...
void TabWnd::DrawListBtn(Graphics& gr, const LPRECT lprcClient)
{
	...
	DrawBtnBkgnd(gr, &rcBtn, bListBtnHilighted);
