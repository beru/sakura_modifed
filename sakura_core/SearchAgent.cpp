	...
	)
{
	...
	// 現在位置の文字の種類によっては選択不能
	if (!bLEFT && WCODE::IsLineDelimiter(pLine[nIdx], GetDllShareData().m_common.edit.m_bEnableExtEol)) {
		return false;
	}
	// 前の単語か？後ろの単語か？
	if (bLEFT) {
...
//	MY_RUNNINGTIMER(runningTimer, "CDocLineMgr::ReplaceData()");
	...
	if (pArg->pInsData && 0 < pArg->pInsData->size()) {
		...
		if (0 < nLen && WCODE::IsLineDelimiter(pInsLine[nLen - 1], GetDllShareData().m_common.edit.m_bEnableExtEol)) {
			// 行挿入
...
prev_line:;
	...
	for (nCount=0; nCount<nInsSize; ++nCount) {
		...
		int nLen = memLine.GetStringLength();
		const wchar_t* pInsLine = memLine.GetStringPtr();
		assert( 0 < nLen && WCODE::IsLineDelimiter(pInsLine[nLen - 1], GetDllShareData().m_common.edit.m_bEnableExtEol) );
#endif
		{
			if (!pDocLine) {
