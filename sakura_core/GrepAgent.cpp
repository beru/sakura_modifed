...
//		m_pOpeBlk = NULL;
	...
	pViewDst->m_nCurSearchKeySequence = GetDllShareData().m_common.search.m_nSearchKeySequence;
	...
	if (bGrepReplace) {
		if (bGrepPaste) {
			...
			bool bColmnSelect;
			bool bLineSelect;
			if (!pViewDst->MyGetClipboardData( memReplace, &bColmnSelect, GetDllShareData().m_common.edit.m_bEnableLineModePaste? &bLineSelect: NULL )) {
	...
	*/
	...
	::CheckDlgButton(hwndCancel, IDC_CHECK_REALTIMEVIEW, GetDllShareData().m_common.search.m_bGrepRealTimeView);	// 2003.06.23 Moca
...
//	SetDrawSwitch(false);
	...
	const bool bDrawSwitchOld = pViewDst->SetDrawSwitch(GetDllShareData().m_common.search.m_bGrepRealTimeView != 0);
	...
	)
{
	...
	// 該当部分
		...
		if (0 < k && WCODE::IsLineDelimiter(pMatchData[ k - 1 ], GetDllShareData().m_common.edit.m_bEnableExtEol)) {
			bEOL = false;
		}
	}
