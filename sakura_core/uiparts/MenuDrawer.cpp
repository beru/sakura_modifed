...
/* 29 */		F_FILENEW_NEWWINDOW			/* , TBSTATE_ENABLED, TBSTATE_BUTTON, 0, 0 */,	// 新規ウィンドウを開く 2009.06.17
...
void MenuDrawer::ResetContents(void)
{
	...
	if (m_pShareData->m_common.window.m_bMenuIcon) {
		// 最低アイコン分の高さを確保
		if (20 > m_nMenuHeight) {
			m_nMenuHeight = 20;
		}
	...
	)
{
	...
	if (nFuncId != 0) {
		...
		// アイコン用ビットマップを持つものは、オーナードロウにする
		{
			...
			if (m_pShareData->m_common.window.m_bMenuIcon || !IsWinVista_or_later()) {
...
void MenuDrawer::DrawItem(DRAWITEMSTRUCT* lpdis)
{
	...
	const bool bMenuIconDraw = !!m_pShareData->m_common.window.m_bMenuIcon;
