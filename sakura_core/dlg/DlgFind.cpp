...
//		EnableItem(IDC_CHK_LOHICASE, FALSE);
	...
	// 検索ダイアログを自動的に閉じる
	CheckButton(IDC_CHECK_bAutoCloseDlgFind, m_pShareData->m_common.search.m_bAutoCloseDlgFind);
...
//	MYTRACE(_T("DlgFind::GetData()"));
	...
	m_pShareData->m_common.search.m_bNOTIFYNOTFOUND = m_bNOTIFYNOTFOUND;	// 検索／置換  見つからないときメッセージを表示
	...
	// 検索ダイアログを自動的に閉じる
	m_pShareData->m_common.search.m_bAutoCloseDlgFind = IsButtonChecked(IDC_CHECK_bAutoCloseDlgFind);
	...
	if (0 < m_strText.length()) {
		...
		if (m_strText.size() < _MAX_PATH) {
			...
			m_pShareData->m_common.search.m_searchOption = m_searchOption;		// 検索オプション
		}
	...
	case IDC_BUTTON_SEARCHPREV:	// 上検索	// Feb. 13, 2001 JEPRO ボタン名を[IDC_BUTTON1]→[IDC_BUTTON_SERACHPREV]に変更
		...
		if (0 < nRet) {
			if (m_bModal) {		// モーダルダイアログか
				...
				// 検索ダイアログを自動的に閉じる
				if (m_pShareData->m_common.search.m_bAutoCloseDlgFind) {
					CloseDialog(0);
				}
			}
		}else if (nRet == 0) {
