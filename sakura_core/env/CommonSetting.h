...
struct CommonSetting_General {
	...
	bool	bNoCaretMoveByActivation;			// マウスクリックにてアクティベートされた時はカーソル位置を移動しない

	// スクロール
	int		nRepeatedScrollLineNum;			// キーリピート時のスクロール行数
	bool	nRepeatedScroll_Smooth;			// キーリピート時のスクロールを滑らかにするか
	int		nPageScrollByWheel;				// キー/マウスボタン + ホイールスクロールでページUP/DOWNする
	int		nHorizontalScrollByWheel;		// キー/マウスボタン + ホイールスクロールで横スクロールする
	...
	// ノーカテゴリ
	bool	bCloseAllConfirm;				// [すべて閉じる]で他に編集用のウィンドウがあれば確認する
...
struct CommonSetting_Window {
	...
	WinSizeMode		eSaveWindowPos;				// ウィンドウ位置継承・固定 WinSizeModeに順ずる
	int				nWinPosX;					// 直接指定 X座標
	int				nWinPosY;					// 直接指定 Y座標

	// ファンクションキー
	int				nFuncKeyWnd_Place;			// ファンクションキー表示位置／0:上 1:下
	int				nFuncKeyWnd_GroupNum;		// ファンクションキーのグループボタン数

	// ルーラー・行番号
	int				nRulerHeight;				// ルーラー高さ
	int				nRulerBottomSpace;			// ルーラーとテキストの隙間
	int				nRulerType;					// ルーラーのタイプ $$$未使用っぽい
	int				nLineNumRightSpace;			// 行番号の右のスペース

	// 分割ウィンドウ
	bool			bSplitterWndHScroll;		// 分割ウィンドウの水平スクロールの同期をとる
	bool			bSplitterWndVScroll;		// 分割ウィンドウの垂直スクロールの同期をとる
	...
	// INI内設定のみ
	int				nVertLineOffset;			// 縦線の描画座標オフセット
...
struct CommonSetting_TabBar {
	bool		bDispTabWnd;					// タブウィンドウ表示する
	bool		bDispTabWndMultiWin;			// タブをまとめない
	...
	wchar_t		szTabWndCaption[MAX_CAPTION_CONF_LEN];	// タブウィンドウキャプション
	bool		bSameTabWidth;					// タブを等幅にする
	bool		bDispTabIcon;					// タブにアイコンを表示する
	DispTabCloseType	dispTabClose;			// タブに閉じるボタンを表示する
	bool		bSortTabList;					// タブ一覧をソートする
	bool		bTab_ListFull;					// タブ一覧をフルパス表示する

	bool		bChgWndByWheel;					// マウスホイールでウィンドウ切り替え
...
struct CommonSetting_Edit {
	...
	bool	bEnableNoSelectCopy;		// 選択なしでコピーを可能にする
	bool	bCopyAndDisablSelection;	// コピーしたら選択解除
	bool	bEnableLineModePaste;		// ラインモード貼り付けを可能にする
	bool	bConvertEOLPaste;			// 改行コードを変換して貼り付ける
...
public:
	...
	UINT	nDropFileNumMax;				// 一度にドロップ可能なファイル数
	bool	bRestoreCurPosition;			// ファイルを開いたときカーソル位置を復元するか
	bool	bRestoreBookmarks;				// ブックマークを復元するかどうか
	bool	bAutoMimeDecode;				// ファイル読み込み時にMIMEのdecodeを行うか
	bool	bQueryIfCodeChange;				// 前回と文字コードが異なるときに問い合わせを行う
	bool	bAlertIfFileNotExist;			// 開こうとしたファイルが存在しないとき警告する
...
public:
	...
	bool		bBackUpDustBox;				// バックアップファイルをごみ箱に放り込む
	bool		bBackUpPathAdvanced;		// バックアップ先フォルダを詳細設定する
	SFilePath	szBackUpPathAdvanced;		// バックアップを作成するフォルダの詳細設定
};
...
struct CommonSetting_Search {
	...
	EncodingType	nGrepCharSet;				// Grep: 文字コードセット
	...
	bool			bCaretTextForSearch;		// カーソル位置の文字列をデフォルトの検索文字列にする
	bool			bInheritKeyOtherView;		// 次・前検索で他のビューの検索条件を引き継ぐ
	TCHAR			szRegexpLib[_MAX_PATH];		// 使用する正規表現DLL

	// Grep
	bool			bGrepExitConfirm;			// Grepモードで保存確認するか
	bool			bGrepRealTimeView;			// Grep結果のリアルタイム表示
...
struct CommonSetting_KeyBind {
	...
	BYTE				keyToKeyNameArr[256 + 10];		// キーコード→割り当て表インデックス
};
...
struct CommonSetting_Helper {
	...
	// キーワードヘルプ
	LOGFONT		lf;								// キーワードヘルプのフォント情報
	INT			nPointSize;						// キーワードヘルプのフォントサイズ（1/10ポイント単位）
...
struct CommonSetting_Macro {
	...
	MacroRec	macroTable[MAX_CUSTMACRO];		// キー割り当て用マクロテーブル
	SFilePath	szMACROFOLDER;					// マクロ用フォルダ
	int			nMacroOnOpened;					// オープン後自動実行マクロ番号
	int			nMacroOnTypeChanged;			// タイプ変更後自動実行マクロ番号
	int			nMacroOnSave;					// 保存前自動実行マクロ番号
	int			nMacroCancelTimer;				// マクロ停止ダイアログ表示待ち時間
};
...
struct CommonSetting_FileName {
	...
	TCHAR	szTransformFileNameTo[MAX_TRANSFORM_FILENAME][_MAX_PATH];		// ファイル名の簡易表示変換後文字列
};
...
struct CommonSetting_OutLine {
	...
	bool		bFunclistSetFocusOnJump;	// フォーカスを移す
	bool		bMarkUpBlankLineEnable;	// 空行を無視する
...
struct CommonSetting {
	...
	//
	CommonSetting_StatusBar			statusBar;			// ステータスバー
	CommonSetting_Plugin			plugin;				// プラグイン
	CommonSetting_MainMenu			mainMenu;			// メインメニュー
};
