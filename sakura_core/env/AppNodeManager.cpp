...
BOOL AppNodeGroupHandle::RequestCloseEditor(EditNode* pWndArr, int nArrCnt, bool bExit, bool bCheckConfirm, HWND hWndFrom)
{
	...
	if (bCheckConfirm && GetDllShareData().m_common.general.bCloseAllConfirm) {	// [すべて閉じる]で他に編集用のウィンドウがあれば確認する
		if (1 < nCloseCount) {
			if (::MYMESSAGEBOX(
				hWndFrom,
	...
	HWND hWndActive;
	bool bTabGroup = (GetDllShareData().m_common.tabBar.bDispTabWnd && !GetDllShareData().m_common.tabBar.bDispTabWndMultiWin);
	if (bTabGroup) {
