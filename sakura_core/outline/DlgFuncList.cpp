...
DWORD DlgFuncList::m_dwDlgTmpSize = 0;
HINSTANCE DlgFuncList::m_lastRcInstance = 0;
	...
	)
{
	...
	m_pFuncInfoArr = pFuncInfoArr;	// 関数情報配列
...
//		::ShowWindow(hwndTree, SW_HIDE);
		...
		for (int i=0; i<m_pFuncInfoArr->GetNum(); ++i) {
			pFuncInfo = m_pFuncInfoArr->GetAt(i);
			if (!bSelected) {
				if (pFuncInfo->m_nFuncLineLAYOUT < nFuncLineTop
					|| (pFuncInfo->m_nFuncLineLAYOUT == nFuncLineTop && pFuncInfo->m_nFuncColLAYOUT <= nFuncColTop)
				) {
					nFuncLineTop = pFuncInfo->m_nFuncLineLAYOUT;
					nFuncColTop = pFuncInfo->m_nFuncColLAYOUT;
					nSelectedLineTop = i;
				}
			}
			{
				if (
					(0
						|| nFuncLineOld < pFuncInfo->m_nFuncLineLAYOUT
						|| (nFuncLineOld == pFuncInfo->m_nFuncColLAYOUT && nFuncColOld <= pFuncInfo->m_nFuncColLAYOUT)
					)
					&& (0
						|| pFuncInfo->m_nFuncLineLAYOUT < m_nCurLine
						|| (pFuncInfo->m_nFuncLineLAYOUT == m_nCurLine && pFuncInfo->m_nFuncColLAYOUT <= m_nCurCol)
					)
				) {
					nFuncLineOld = pFuncInfo->m_nFuncLineLAYOUT;
					nFuncColOld = pFuncInfo->m_nFuncColLAYOUT;
					bSelected = true;
					nSelectedLine = i;
				}
			}
		...
		for (int i=0; i<m_pFuncInfoArr->GetNum(); ++i) {
			...
			if (m_bLineNumIsCRLF) {
				auto_sprintf(szText, _T("%d"), pFuncInfo->m_nFuncLineCRLF);
			}else {
				auto_sprintf(szText, _T("%d"), pFuncInfo->m_nFuncLineLAYOUT);
			}
			...
			// 行番号の表示 false=折り返し単位／true=改行単位
			if (m_bLineNumIsCRLF) {
				auto_sprintf(szText, _T("%d"), pFuncInfo->m_nFuncColCRLF);
			}else {
				auto_sprintf(szText, _T("%d"), pFuncInfo->m_nFuncColLAYOUT);
			}
			item.mask = LVIF_TEXT;
			item.pszText = szText;
			item.iItem = i;
			...
			item.mask = LVIF_TEXT;
			item.pszText = const_cast<TCHAR*>(pFuncInfo->m_memFuncName.GetStringPtr());
			item.iItem = i;
			...
			item.mask = LVIF_TEXT;
			if (pFuncInfo->m_nInfo == 1) {item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK01));}else
			if (pFuncInfo->m_nInfo == 10) {item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK02));}else
			if (pFuncInfo->m_nInfo == 20) {item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK03));}else
			if (pFuncInfo->m_nInfo == 11) {item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK04));}else
			if (pFuncInfo->m_nInfo == 21) {item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK05));}else
			if (pFuncInfo->m_nInfo == 31) {item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK06));}else
			if (pFuncInfo->m_nInfo == 41) {item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK07));}else
			if (pFuncInfo->m_nInfo == 50) {item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK08));}else
			if (pFuncInfo->m_nInfo == 51) {item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK09));}else
			if (pFuncInfo->m_nInfo == 52) {item.pszText = const_cast<TCHAR*>(LS(STR_DLGFNCLST_REMARK10));}else {
			...
			if (item.pszText[0] != _T('\0')) {
				...
				auto_sprintf(
					...
					pFuncInfo->m_nFuncLineCRLF,			// 検出行番号
					pFuncInfo->m_nFuncColCRLF				// 検出桁番号
				);
	...
	CheckButton(IDC_CHECK_bAutoCloseDlgFuncList, m_pShareData->m_common.outline.bAutoCloseDlgFuncList);
	// アウトライン ブックマーク一覧で空行を無視する
	CheckButton(IDC_CHECK_bMarkUpBlankLineEnable, m_pShareData->m_common.outline.bMarkUpBlankLineEnable);
	// アウトライン ジャンプしたらフォーカスを移す
	CheckButton(IDC_CHECK_bFunclistSetFocusOnJump, m_pShareData->m_common.outline.bFunclistSetFocusOnJump);

	// アウトライン ■位置とサイズを記憶する // 20060201 aroka
	CheckButton(IDC_BUTTON_WINSIZE, m_pShareData->m_common.outline.bRememberOutlineWindowPos);
	// ボタンが押されているかはっきりさせる 2008/6/5 Uchi
	SetItemText(IDC_BUTTON_WINSIZE, 
		m_pShareData->m_common.outline.bRememberOutlineWindowPos ? _T("■") : _T("□"));

	// ダイアログを自動的に閉じるならフォーカス移動オプションは関係ない
	EnableItem(IDC_CHECK_bFunclistSetFocusOnJump, !m_pShareData->m_common.outline.bAutoCloseDlgFuncList);
	...
	)
{
	...
	for (int i=0; i<m_pFuncInfoArr->GetNum(); ++i) {
		const FuncInfo* pFuncInfo = m_pFuncInfoArr->GetAt(i);
		const TCHAR* pWork = pFuncInfo->m_memFuncName.GetStringPtr();
		int m = 0;
		int nClassNest = 0;
		// クラス名::メソッドの場合
		...
		if (0 < nClassNest) {
			...
			for (k=0; k<nClassNest; ++k) {
				...
				if (!htiClass) {
					...
					tvis.item.lParam = -1;
					if (bAddClass) {
						if (pFuncInfo->m_nInfo == FL_OBJ_NAMESPACE) {
			...
			if (FL_OBJ_CLASS <= pFuncInfo->m_nInfo  && pFuncInfo->m_nInfo <= FL_OBJ_ELEMENT_MAX) {
				htiClass = TVI_ROOT;
			}else {
		...
		case FL_OBJ_GLOBAL:			//「グローバル」は別の場所で処理してるので除外
			break;
		default:
			_tcscat(pFuncName, to_tchar(m_pFuncInfoArr->GetAppendText(pFuncInfo->m_nInfo).c_str()));
		}
		...
		{
			if ((nFuncLineOld < pFuncInfo->m_nFuncLineLAYOUT
				|| (nFuncLineOld == pFuncInfo->m_nFuncColLAYOUT && nFuncColOld <= pFuncInfo->m_nFuncColLAYOUT))
			  && (pFuncInfo->m_nFuncLineLAYOUT < m_nCurLine
				|| (pFuncInfo->m_nFuncLineLAYOUT == m_nCurLine && pFuncInfo->m_nFuncColLAYOUT <= m_nCurCol))
			) {
...
void DlgFuncList::SetTree(bool tagjump, bool nolabel)
{
	...
	for (int i=0; i<nFuncInfoArrNum; ++i) {
		FuncInfo* pFuncInfo = m_pFuncInfoArr->GetAt(i);
		...
		*/
		...
		cTVInsertStruct.item.pszText = pFuncInfo->m_memFuncName.GetStringPtr();
		...
		if (pFuncInfo->m_memFileName.GetStringPtr() && m_pFuncInfoArr->m_szFilePath[0]) {
			if (auto_stricmp(pFuncInfo->m_memFileName.GetStringPtr(), m_pFuncInfoArr->m_szFilePath.c_str()) == 0) {
				bFileSelect = true;
			}
		}else {
			bFileSelect = true;
		...
		if (pFuncInfo->IsAddClipText()) {
			...
			if (tagjump) {
				...
				text.AllocStringBuffer(
					  pFuncInfo->m_memFuncName.GetStringLength()
					+ nStackPointer * 2 + 1
					+ _tcslen(pszFileName)
					+ 20
				);
	...
	)
{
	...
	EditView* pEditView = (EditView*)m_lParam;
	if (pEditView) {
		if (!IsDocking() && m_pShareData->m_common.outline.bRememberOutlineWindowPos) {
			WINDOWPLACEMENT windowPlacement;
			windowPlacement.length = sizeof(windowPlacement);
			if (::GetWindowPlacement(pEditView->m_pEditWnd->GetHwnd(), &windowPlacement)) {
				// ウィンドウ位置・サイズを-1以外の値にしておくと、Dialogで使用される．
				m_xPos = m_pShareData->m_common.outline.xOutlineWindowPos + windowPlacement.rcNormalPosition.left;
				m_yPos = m_pShareData->m_common.outline.yOutlineWindowPos + windowPlacement.rcNormalPosition.top;
				m_nWidth =  m_pShareData->m_common.outline.widthOutlineWindow;
				m_nHeight = m_pShareData->m_common.outline.heightOutlineWindow;
			}
		}else if (IsDocking()) {
			m_xPos = 0;
			m_yPos = 0;
...
BOOL DlgFuncList::OnBnClicked(int wID)
{
	...
	case IDC_BUTTON_WINSIZE:
		{// ウィンドウの位置とサイズを記憶 // 20060201 aroka
			m_pShareData->m_common.outline.bRememberOutlineWindowPos = IsButtonChecked(IDC_BUTTON_WINSIZE);
		}
		// ボタンが押されているかはっきりさせる 2008/6/5 Uchi
		SetItemText(IDC_BUTTON_WINSIZE,
			m_pShareData->m_common.outline.bRememberOutlineWindowPos ? _T("■") : _T("□"));
	...
	case IDC_CHECK_bFunclistSetFocusOnJump:
		m_pShareData->m_common.outline.bAutoCloseDlgFuncList = IsButtonChecked(IDC_CHECK_bAutoCloseDlgFuncList);
		m_pShareData->m_common.outline.bMarkUpBlankLineEnable = IsButtonChecked(IDC_CHECK_bMarkUpBlankLineEnable);
		m_pShareData->m_common.outline.bFunclistSetFocusOnJump = IsButtonChecked(IDC_CHECK_bFunclistSetFocusOnJump);
		EnableItem(IDC_CHECK_bFunclistSetFocusOnJump, !m_pShareData->m_common.outline.bAutoCloseDlgFuncList);
	...
	if (hwndTree == pnmh->hwndFrom) {
		...
		case NM_KILLFOCUS:
			...
			if (m_bWaitTreeProcess) {
				if (m_pShareData->m_common.outline.bFunclistSetFocusOnJump) {
...
BOOL DlgFuncList::OnDestroy(void)
{
	...
	if (!IsDocking() && m_pShareData->m_common.outline.bRememberOutlineWindowPos) {
		...
		if (::GetWindowPlacement(hwndEdit, &windowPlacement)) {
			// ウィンドウ位置・サイズを記憶
			m_pShareData->m_common.outline.xOutlineWindowPos = m_xPos - windowPlacement.rcNormalPosition.left;
			m_pShareData->m_common.outline.yOutlineWindowPos = m_yPos - windowPlacement.rcNormalPosition.top;
			m_pShareData->m_common.outline.widthOutlineWindow = m_nWidth;
			m_pShareData->m_common.outline.heightOutlineWindow = m_nHeight;
		}
	}
	...
	if (IsDocking() && hwndEdit) {
		pEditView->m_pEditWnd->EndLayoutBars();
	}
	...
	if (0 < GetData() && (m_funcInfo || 0 < m_sJumpFile.size() )) {
		if (m_bModal) {		// モーダル ダイアログか
			...
			if (bCheckAutoClose && bFileJumpSelf) {
				...
				}else if (m_pShareData->m_common.outline.bAutoCloseDlgFuncList) {
					::DestroyWindow( GetHwnd() );
				}else if (m_pShareData->m_common.outline.bFunclistSetFocusOnJump) {
					::SetFocus( ((EditView*)m_lParam)->GetHwnd() );
				}
			}
		}
...
void DlgFuncList::GetDockSpaceRect(LPRECT pRect)
{
	...
	hwnd[nCount] = pEditView->m_pEditWnd->GetMiniMap().GetHwnd();
	if (hwnd[nCount]) {
		++nCount;
	}
	...
	)
{
	if (wParam == 2) {
		...
		if (m_pszTimerJumpFile) {
			...
			m_pszTimerJumpFile = NULL;
			bool bSelf = false;
			pView->TagJumpSub( pszFile, m_pointTimerJump, false, false, &bSelf );
...
bool DlgFuncList::ChangeLayout(int nId)
{
	...
	if (!GetHwnd()) {	// 現在は非表示
		...
		pEditView->m_pEditWnd->EndLayoutBars(m_bEditWndReady);
	}
	return false;
}
	...
	)
{
	...
	if (!IsDocking()) {	// フローティング → フローティング
		...
		*pRect = rcWnd;
		...
		rcFloat.left = 0;
		rcFloat.top = 0;
		if (m_pShareData->m_common.outline.bRememberOutlineWindowPos
				&& m_pShareData->m_common.outline.widthOutlineWindow	// 初期値だと 0 になっている
				&& m_pShareData->m_common.outline.heightOutlineWindow	// 初期値だと 0 になっている
		) {
			// 記憶しているサイズ
			rcFloat.right = m_pShareData->m_common.outline.widthOutlineWindow;
			rcFloat.bottom = m_pShareData->m_common.outline.heightOutlineWindow;
